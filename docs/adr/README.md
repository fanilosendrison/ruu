...
- [ADR-083 — Allow shared governance implementation without transferring repository authority](adr-083-allow-shared-governance-implementation-without-transferring-repository-authority.md) — Accepted — amends ADR-082 by separating repository authority and qualification evidence from reusable governance implementation; pinned proto-ring tooling may provide shared mechanisms after parity is demonstrated, while Ruu keeps its local profile, overlay, bindings, evidence, generated outputs, and semantic authority.

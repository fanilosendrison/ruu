#!/usr/bin/env python3
"""Global regression/state-space audit through ADR-046 PromotionGroup resolution semantics.

This audit does not claim to enumerate unbounded Git DAG/provider states. It
revalidates the finite current architecture families from v15 and adds explicit
coverage for ADR-046 durable pre-exact PromotionGroups: canonical content-addressed
logical membership, trigger independence, closed-set completeness, all-or-nothing
READY_INTERNAL resolution, snapshot/CAS invalidation, explicit singleton semantics,
and deterministic materialization into ADR-045 exact PromotionUnits.
"""
...
def _promotion_unit_v1_id(members):
    """Reference audit encoding for ADR-045's domain/version/content identity rule."""
...
def audit_promotion_unit_content_address_identity():
    ...
    invalid = [(), ("repo:R1/cu:A/state:111", "repo:R1/cu:A/state:111")]
...
def audit_promotion_group_to_promotion_unit_materialization():
    logical_members = [
        ("repo:R1/cu:A",),
    ...
    revision = ["REV1", "REV2"]
    n = 0
    for logical, rev in product(logical_members, revision):
        n += 1
        gid = _promotion_group_v1_id(logical)
        assert gid
        exact = tuple(f"{m}/state:{rev}" for m in logical)
        pid = _promotion_unit_v1_id(exact)
        assert pid
        # Group identity is stable across exact-state revisions.
        assert _promotion_group_v1_id(logical) == gid
        if rev == "REV2":
            prior = tuple(f"{m}/state:REV1" for m in logical)
...
def static_consistency_checks():
    ...
    assert nums == list(range(1, 47)), nums
    ...
    required_control = [
        ...
        "Promotion topology/dependency intent remains a separate relationship",
    ...
    assert "Exact multi-source projection/materialization remains open under backlog item 30.18" in control
...
def main():
    families = {
        ...
        "promotion_group_to_promotion_unit_materialization": audit_promotion_group_to_promotion_unit_materialization(),
    ...
    print("Ruu state-space audit v16: PASS")

#!/usr/bin/env python3
"""Incremental global regression/state-space audit through ADR-067.

Retains the v32 finite regression count, replaces the invalid ADR-065 realization
family with route-conformant ADR-066 proof/recovery families, adds ADR-067 supersession,
performs current normative static checks, and reruns the concrete Git primitive smokes.
"""
...
assert nums==list(range(1,68)), nums
...
assert sorted(set(opens))==['30.41'], opens
...
assert 'STATE-SPACE-AUDIT-v34.md' in main
assert (ROOT/'STATE-SPACE-AUDIT-v34.md').exists()
assert '## 30.34 Resolved by ADR-065 as corrected by ADR-066' in main
assert '30.41' in backlog
assert '## 30.41 OPEN — ConvergenceUnit abandonment/disposition after PromotionGroup binding' in main
...
changed=len(realization_combos)+len(recovery_combos)+len(supersession_combos)
...
print(f'ADR-066 route-conformant realization finite family: PASS ({len(realization_combos)} combinations)')
print(f'ADR-066 policy-drift/commitment recovery finite family: PASS ({len(recovery_combos)} combinations)')
print(f'ADR-067 supersession finite family: PASS ({len(supersession_combos)} combinations)')
print(f'retained v32 finite combinations: {RETAINED_V32}')
print(f'v34 changed/revalidated finite combinations: {RETAINED_V32+changed}')
print('ADR numbering: PASS (1..67)')

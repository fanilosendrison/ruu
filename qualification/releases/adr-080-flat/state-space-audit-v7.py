#!/usr/bin/env python3
"""Global regression/state-space audit for ADR-037 eager ContributionUnit integration and sealed ConvergenceUnit readiness model.
...
changed by ADR-035/ADR-036/ADR-037, reruns the finite ADR-033/029..032 families, and performs
...
def audit_eager_contribution_integration():
    lifecycle = ["OPEN", "CLOSED", "ABANDONED", "REMOVED"]
    checkpoint = ["VALID_MANAGED", "INVALID", "STALE"]
    topology = ["KNOWN", "UNKNOWN"]
    claim = ["HELD", "OTHER", "NONE"]
    ...
    n = 0
    baseline = {}
    for life, cp, topo, cl, cf, rel, ev, release in product(
        lifecycle, checkpoint, topology, claim, conflict, relation, evidence, legacy_release
    ):
        n += 1
        eager_candidate = (
            life in {"OPEN", "CLOSED"}
            and cp == "VALID_MANAGED"
            and topo == "KNOWN"
            and cl == "HELD"
            and cf == "CLEAR"
            and rel != "UNKNOWN"
        ...
        # Legacy release state is intentionally irrelevant to both predicates.
        ...
        if key in baseline:
            ...
            baseline[key] = value
        if life in {"ABANDONED", "REMOVED"} or cp != "VALID_MANAGED" or topo != "KNOWN" or cl != "HELD" or cf != "CLEAR":
            assert not eager_candidate
        if may_ff:
            assert life in {"OPEN", "CLOSED"} and ev == "VALID"
        if must_sync_first:
            assert rel == "NEEDS_SYNC"
        _ = release
    return n
...
def audit_convergence_membership_readiness():
    ...
    retained_refs = ["NONE", "TERMINAL_RESOLVED_RETAINED"]
    n = 0
    for mem, contrib, internal_state, ev, fp, refs in product(
        membership, contributions, internal, evidence, fixed_point, retained_refs
    ):
        n += 1
        ready = (
            mem == "SEALED"
            and contrib == "ALL_TERMINAL_RESOLVED"
            and internal_state == "CLEAR"
            and ev == "VALID"
            and fp == "YES"
        )
        if mem == "OPEN" or contrib != "ALL_TERMINAL_RESOLVED" or internal_state != "CLEAR" or ev != "VALID" or fp != "YES":
            assert not ready
        # Historical terminal/resolved ref retention alone may not change readiness.
        if ready:
            assert refs in {"NONE", "TERMINAL_RESOLVED_RETAINED"}
        _ = refs
    return n
...
def static_consistency_checks():
    ...
    assert nums == list(range(1, 38)), nums
    ...
    required_main = [
        ...
        "Experimental/alternative isolation",
        "membership SEALED",
        "OPEN | SEALED",
        "Physical ContributionUnit ref deletion is not itself the readiness criterion",
    ]
    ...
    for p in adr_files:
        if p.name.startswith("ADR-034-") or p.name.startswith("ADR-035-") or p.name.startswith("ADR-036-") or p.name.startswith("ADR-037-"):
            continue
    ...
    assert "Contribution-to-convergence semantics are closed by ADR-037" in backlog
    assert "## 30.5 Resolved by ADR-037" in main
    assert "## 30.6 Resolved by ADR-037" in main
    assert "## 30.7 Resolved by ADR-037" in main
    assert "5. who creates/chooses a convergence unit" not in backlog
    assert "6. contribution-unit→convergence-unit readiness signal" not in backlog
    assert "7. additional `READY_INTERNAL` evidence" not in backlog
    assert "No separate ContributionUnit integration-release/readiness state exists" in adr37
    assert "Experimental/alternative isolation" in adr37
    assert "contribution membership == SEALED" in adr37
    assert "zero contribution-unit refs" not in main
    assert "exact contribution unit-integration readiness" not in main
    assert "contribution-unit readiness VALID_EXACT" not in main
...
def main():
    ...
    print("Ruu state-space audit v7: PASS")

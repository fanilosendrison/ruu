...
Repository admission/reactivation before new managed writes is an External Control Plane responsibility under [`EXTERNAL-CONTROL-PLANE-CONTRACT.md`](EXTERNAL-CONTROL-PLANE-CONTRACT.md). The durable registry remains a shared Git coordination substrate consumed by `ruu`; `ACTIVE_CONVERGENCE_SET` remains only a derived acceleration index under ADR-036.

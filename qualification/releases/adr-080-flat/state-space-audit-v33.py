#!/usr/bin/env python3
# SUPERSEDED HISTORICAL AUDIT: v34 replaces this proof model after hostile review.
"""Incremental global regression/state-space audit through ADR-065.

Retains the v32 finite regression count, adds the ADR-065 exact final target-realization
proof family, performs current normative static checks, and reruns the five concrete
Git primitive smokes.
"""
...
def smoke_provider_realization():
    script=ROOT/'git-provider-realization-smoke-v1.sh'
    assert script.exists()
    r=run([str(script)],cwd=ROOT)
    assert r.returncode==0
    assert 'provider-result realization smoke: PASS' in r.stdout
...
# ADR-065 exact final target-realization proof family: 2*3*2*2*2 = 48.
realization_routes=['DIRECT_TARGET_ADVANCE','PROVIDER_SUBMISSION']
candidate_target_relation=['EXACT','ANCESTOR','ABSENT']
provider_binding=['EXACT_C_TO_R','MISSING_OR_MISMATCHED']
provider_finalized=[False,True]
result_target_relation=['PRESENT','ABSENT']
realization_combos=list(itertools.product(realization_routes,candidate_target_relation,provider_binding,provider_finalized,result_target_relation))
assert len(realization_combos)==48
realization_outcomes=set()
for route,c_rel,binding,finalized,r_rel in realization_combos:
    if c_rel in ('EXACT','ANCESTOR'):
        outcome='PROVEN_NATIVE_GIT'
    elif route=='DIRECT_TARGET_ADVANCE':
        outcome='TARGET_REALIZATION_UNPROVEN'
    elif finalized and binding=='EXACT_C_TO_R' and r_rel=='PRESENT':
        outcome='PROVEN_PROVIDER_RESULT'
    else:
        outcome='TARGET_REALIZATION_UNPROVEN'
    realization_outcomes.add(outcome)
    # A provider completion event alone never proves a rewrite.
    if route=='PROVIDER_SUBMISSION' and c_rel=='ABSENT' and finalized and binding!='EXACT_C_TO_R':
        assert outcome=='TARGET_REALIZATION_UNPROVEN'
    # Exact C→R without R in target is still unproven.
    if route=='PROVIDER_SUBMISSION' and c_rel=='ABSENT' and finalized and binding=='EXACT_C_TO_R' and r_rel=='ABSENT':
        assert outcome=='TARGET_REALIZATION_UNPROVEN'
    # Direct route never consumes provider rewrite evidence.
    if route=='DIRECT_TARGET_ADVANCE' and c_rel=='ABSENT':
        assert outcome=='TARGET_REALIZATION_UNPROVEN'
assert 'PROVEN_NATIVE_GIT' in realization_outcomes
assert 'PROVEN_PROVIDER_RESULT' in realization_outcomes
assert 'TARGET_REALIZATION_UNPROVEN' in realization_outcomes

# Static current-doc checks.
adrs=sorted(ROOT.glob('ADR-*.md'))
nums=[int(x.name[4:7]) for x in adrs]
assert nums==list(range(1,66)), nums
...
assert 'PromotionRealizationProof' in adr65
assert 'ProviderFinalizationObservation' in adr65
assert '### 5. Provider `MERGED` / `FINALIZED` state alone never proves promotion' in adr65
...
assert 'Invariant 148 — Promotion is terminal only after exact target realization proof' in main
assert 'Invariant 149 — Native Git ancestry is sufficient realization proof when preserved' in main
assert 'Invariant 150 — Provider rewrite proof binds exact C to exact R and independently observes R in target history' in main
assert 'Invariant 151 — Rewritten-result semantic equivalence is not a Ruu prerequisite' in main
assert 'frozen checkpoint-offer boundary' in contract
assert '### 2.16 Provider transformation / final target realization trust boundary' in contract
assert 'parent(K)=P and tree(K)=T' in main
assert 'STATE-SPACE-AUDIT-v33.md' in main
assert (ROOT/'STATE-SPACE-AUDIT-v33.md').exists()

assert '## 30.34 Resolved by ADR-065 — exact final target realization proof' in main
assert 'No genuine core design question currently remains open' in backlog
assert 'provider `MERGED` / `FINALIZED` observation is never by itself sufficient' in main
assert 'does not require generic diff/patch/semantic equivalence' in main
...
smokes=[
 ...
 ('provider-result realization v1',smoke_provider_realization),
]
...
changed=len(realization_combos)
...
print(f'ADR-065 final target-realization proof finite family: PASS ({len(realization_combos)} combinations)')
print(f'retained v32 finite combinations: {RETAINED_V32}')
print(f'v33 changed/revalidated finite combinations: {RETAINED_V32+changed}')
print('ADR numbering: PASS (1..65)')

#!/usr/bin/env python3
"""Global regression/state-space audit through ADR-047 repository-local PromotionGroup projection semantics.
...
coverage for ADR-047 deterministic repository partitioning of completely resolved
PromotionGroups, repository-local PromotionUnit validity, complete resolution-map
adoption, same-group/same-repository non-splitting, and cross-repository aggregate
progress while preserving every prior ADR-046 regression family.
"""
...
def static_consistency_checks():
    ...
    required_main = [
        ...
        "Invariant 47I — Repository projection does not synthesize PromotionTopology",
    ...
    assert "39. repository-local multi-source candidate/head materialization." in backlog
    ...
    required_adr47 = [
        ...
        "PromotionTopology remains separate and is never synthesized by projection",
        "30.39 — repository-local multi-source candidate materialization",
    ]
    ...
    assert "deterministic repository-local group projection is closed by ADR-047" in control
    assert "Repository-local multi-source candidate/head materialization remains open under backlog item 30.39" in control
...
def main():
    ...
    print("Ruu state-space audit v17: PASS")

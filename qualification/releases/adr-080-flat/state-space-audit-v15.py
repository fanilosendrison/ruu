#!/usr/bin/env python3
"""Global regression/state-space audit through ADR-045 PromotionUnit identity semantics.

This audit does not claim to enumerate unbounded Git DAG/provider states. It
revalidates the finite current architecture families from v14 and adds explicit
coverage for ADR-045 immutable content-addressed PromotionUnits: canonical
unordered-set identity, idempotent redeclaration, exact-member change identity,
identity independence from lifecycle/topology/policy/semantic labels, structural
validity versus current promotion eligibility, and fail-closed persisted
identity/definition mismatch.
"""
...
def static_consistency_checks():
    ...
    required_control = [
        ...
        "PROTECTED_EXTERNAL",
        "DeclarePromotionUnit(",
        "Set<ExactConvergenceStateRef>",
        "content-addressed promotion_unit_id",
        "Promotion topology/dependency intent is supplied separately",
    ...
    assert "Default mapping and multi-source projection remain open under backlog items 30.17–30.18" in control
...
def main():
    ...
    print("Ruu state-space audit v15: PASS")

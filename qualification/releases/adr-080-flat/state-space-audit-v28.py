...
def static_consistency_checks():
    ...
    assert nums == list(range(1, 60)), nums

    main = (ROOT / "ruu-requirements-v1-merge-policy.md").read_text()
    backlog = (ROOT / "OPEN-DESIGN-BACKLOG.md").read_text()
    adr5 = (ROOT / "ADR-005-coordinate-simultaneous-convergers-with-fine-grained-claims-and-cas.md").read_text()
    adr7 = (ROOT / "ADR-007-converge-every-edge-bidirectionally-to-a-fixed-point.md").read_text()
    ...
    adr54 = (ROOT / "ADR-054-separate-promotion-unit-completion-convergence-unit-closure-and-retirement.md").read_text()
    adr55 = (ROOT / "ADR-055-settle-partial-cross-repository-ships-with-forward-actions-and-publication-episodes.md").read_text()
    adr56 = (ROOT / "ADR-056-bootstrap-new-repositories-before-managed-authoring-and-keep-provider-creation-external.md").read_text()
    adr57 = (ROOT / "ADR-057-externalize-development-verification-and-model-ruu-as-state-dependent-git-progression.md").read_text()
    adr58 = (ROOT / "ADR-058-preserve-native-git-equivalence-and-whole-surface-checkpoint-intent.md").read_text()
    adr59 = (ROOT / "ADR-059-canonicalize-whole-surface-checkpoints-with-native-git-tree-construction.md").read_text()
    adr29 = (ROOT / "ADR-029-require-full-verification-before-authoritative-managed-state-adoption.md").read_text()
    adr30 = (ROOT / "ADR-030-bind-full-verification-evidence-to-exact-state-and-reuse-it-when-valid.md").read_text()
    adr31 = (ROOT / "ADR-031-separate-verification-capacity-from-git-mutation-claims.md").read_text()
    ...
    required_main = [
        ...
        "state-dependent governed super Git command",
        "DevelopmentValidationDemand",
        "DevelopmentValidationEvidence",
        "Development-validation execution is outside Git mutation authority",
        "Development-verification executor/scheduler — intentionally absent",
        "Resolved by ADR-059 — canonical whole-editing-surface checkpoint snapshot and identity",
        "Final integration / merge-result proof",
    ]
    ...
    assert "New-repository creation/bootstrap authority is closed by ADR-056" in backlog
    assert "26. new repository creation/provisioning authority." not in backlog
    ...
    required_control = [
        ...
        "RepositoryBootstrapContract",
        "RepositoryCreationPolicy",
        "REPOSITORY_ADMITTED",
        "exact non-null bootstrap OID B0",
        "built-in safe default is `PRIVATE`",
        "Provider attachment may be deferred",
        "filesystem paths and provider names are not identity",
        "does not authorize destructive repository deletion",
        "Development-validation demand and exact evidence",
        "does not run or schedule repository tests",
        "DevelopmentValidationDemand",
        "DevelopmentValidationEvidence",
        "Validation execution capacity, retries, fixed-point mutation handling",
    ]
    ...
    required_adr48 = [
        ...
        "The exact final candidate may require exact external development validation before adoption",
    ...
    required_adr49 = [
        ...
        "Provider submission identity persists across exact revisions within one publication episode",
    ...
    required_adr50 = [
        ...
        "Every new exact restacked head is development-validated",
    ...
    required_adr54 = [
        ...
        "PromotionUnit `PROMOTED` means only that exact snapshot reached its terminal publication outcome",
        "A ConvergenceUnit stays reactivable while any relevant logical ship remains nonterminal",
        "ConvergenceUnit `PROMOTED` is the semantic no-more-authoring boundary",
        "`PROMOTED` is still not `RETIRED`",
        "Retirement and physical ref deletion are separate",
        "V1 built-in historical internal-ref retention is `KEEP`",
        "Durable logical/audit history is independent from physical ref retention",
        "Multiple/superseding PromotionGroups are handled by the universal quantifier",
        "The resulting lifecycle has three distinct completion meanings",
    ]
    for marker54 in required_adr54:
        assert marker54 in adr54, marker54

    required_adr55 = [
        "Backlog item **30.25 is closed**",
        "`PARTIALLY_PROMOTED` is nonterminal progress, not automatically a settlement incident",
        "Non-nominal partial progress creates one durable exact-generation settlement demand",
        "Settlement semantics belong to the External Control Plane / Development System",
        "All settlement Git effects are forward-only",
        "Stable logical submission identity may contain multiple publication episodes",
        "A terminal provider episode is never reopened or mutated",
        "Roll-forward terminal settlement is `ALL_PROMOTED` over the current exact group resolution",
        "Compensation is an explicit terminal semantic settlement, not an inferred rollback",
        "Partial promotion is never silently abandoned",
        "Settlement demands are global nonterminal managed obligations",
        "ADR-039 contract is retroactively consolidated for accepted pre-039 boundaries",
    ]
    for marker55 in required_adr55:
        assert marker55 in adr55, marker55
    ...
    assert "24. promotion-unit completion vs convergence-unit retirement;" not in backlog
    assert "25. cross-repository External Control Plane compensation;" not in backlog
    ...
    assert "PromotionUnit completion / ConvergenceUnit closure-retirement semantics are closed by ADR-054" in backlog
    assert "Cross-repository settlement/compensation semantics are closed by ADR-055" in backlog
    ...
    assert "## 30.24 Promotion-unit completion versus convergence-unit retirement" not in main
    ...
    assert "ADR-054 closes backlog 30.24" in control
    assert "ADR-055 closes backlog 30.25" in control
    assert "ADR-056 closes brand-new repository/provider-repository creation" in control
    assert "Backlog item **30.26 is closed**" in adr56
    assert "V1 does not admit an unborn/null target" in adr56
    assert "Local repository creation and provider repository creation are distinct" in adr56
    assert "RepositoryProvisioningOperation" in adr56
    assert "The system MUST NOT silently adopt an unrelated preexisting repository" in adr56
    assert "A provisioning failure does not itself authorize that deletion" in adr56
    assert "ACTIVE_CONVERGENCE_SET` to hide obligations" in control
    assert "trigger carries no ContributionUnit mutation authority, no semantic work ownership" in control
    assert "PR review-request publication-intent boundary" in control
    assert "Runtime requests cannot manufacture promotion authorization" in control
    assert "PublicationEpisodes" in control
    ...
    assert "→ local target FF to C" not in main
    ...
    assert (ROOT / "git-repository-bootstrap-smoke-v1.sh").exists()
    assert "PASS" in (ROOT / "git-repository-bootstrap-smoke-v1.txt").read_text()

    # ADR-057 boundary / backlog / absence checks.
    required_adr57 = [
        "state-dependent, governed super Git operation",
        "there is demand to make currently declared/transferable managed work progress through its next legal Git boundaries.",
        "Development verification execution belongs outside `ruu`",
        "Git/convergence validation remains inside `ruu`",
        "DevelopmentValidationEvidence",
        "DevelopmentValidationDemand",
        "Ruu-synthesized candidates are immutable validation subjects",
        "ADR-031 verification-capacity scheduling is therefore superseded as a `ruu` responsibility",
        "30.27** remains open only for the genuinely Git-level **checkpoint candidate snapshot/membership contract**",
        "30.29** verification executor/capacity scheduling — closed as outside `ruu`",
        "30.34** final integration / merge-result proof remains a genuine open `ruu` question",
    ]
    for marker57 in required_adr57:
        assert marker57 in adr57, marker57

    assert "## Current normative reading after ADR-057" in adr29
    assert "does not run tests to satisfy that demand" in adr29
    assert "## Current normative reading after ADR-057" in adr30
    assert "historical internal verifier/fixed-point algorithm does not" in adr30
    assert "## Current normative reading after ADR-057" in adr31
    assert "superseded as a `ruu` component" in adr31

    assert "## 30.27 Resolved by ADR-059" in main
    assert "## 30.28 External DevelopmentValidationEvidence reference/binding contract" in main
    assert "## 30.29 Closed by ADR-057" in main
    assert "## 30.30 Closed by ADR-057" in main
    assert "## 30.31 Closed by ADR-057" in main
    assert "## 30.32 Closed by ADR-057" in main
    assert "## 30.33 Closed by ADR-057" in main
    assert "## 30.34 Final integration / merge-result proof" in main
    assert "## 30.35 Closed by ADR-057" in main
    assert "## 30.36 REVIEW_NOT_REQUESTED" in main
    assert "## 30.37 Closed by ADR-057" in main
    assert "## 30.38 Closed by ADR-057" in main

    assert "Checkpoint candidate canonical snapshot / identity — closed by ADR-059 (30.27)" in backlog
    assert "External DevelopmentValidationEvidence reference/binding — open (30.28)" in backlog
    assert "Final target integration / merge-result proof — open (30.34)" in backlog
    assert "REVIEW_NOT_REQUESTED policy fallback — open (30.36)" in backlog
    assert "former 30.29, 30.30, 30.31, 30.35, 30.37" in backlog

    required_adr58 = [
        "Native-Git equivalence is a global invariant",
        "Native operability does not imply managed authorization",
        "Checkpoint invocation has whole-editing-surface intent",
        "Existing staging is non-authoritative for checkpoint membership",
        "partial checkpoint selection: NO in v1",
        "native-Git postcondition/equivalence: REQUIRED",
    ]
    for marker58 in required_adr58:
        assert marker58 in adr58 or marker58 in backlog, marker58
    assert "Invariant 131 — Managed Git state remains natively interpretable" in main
    assert "Invariant 132 — Native Git operability is not governance bypass" in main
    assert "Invariant 133 — Checkpoint intent is whole-editing-surface, not partial selection" in main
    assert "Invariant 134 — Staging does not select managed checkpoint membership" in main
    assert "Invariant 135 — Canonical checkpoint trees come from the exact parent plus the complete observable surface" in main
    assert "Invariant 136 — Non-ignored untracked work is checkpoint work" in main
    assert "Invariant 137 — Ambiguous or incompletely observable Git state is not guessed" in main
    assert "Invariant 138 — Tree-equal checkpoint progression is a no-op" in main
    assert "Invariant 139 — Checkpoint candidate identity is exact Git state, attribution is separate" in main
    assert "staged vs unstaged tracked changes;" not in backlog
    assert "partial-staging support or rejection;" not in backlog
    assert "Still open only for the canonical Git-relevant snapshot mechanics" not in backlog

    required_adr59 = [
        "canonical native Git tree T",
        "untracked non-ignored path",
        "Ignored untracked paths are excluded",
        "Unmerged/in-progress Git state blocks ordinary checkpoint collection",
        "dirty/unknown submodule state blocks",
        "V1 checkpoint collection requires a fully observable editing surface",
        "Tree equality with the parent is a no-op",
    ...
    for marker59 in required_adr59:
        assert marker59 in adr59, marker59
    assert "## Current normative reading after ADR-059" in adr58
    assert "## Current normative reading after ADR-059" in adr57
    ...
    for marker in forbidden_executor_state:
        assert marker not in main, marker

    # The core may mention executor/scheduler terms only to state their intentional absence.
    assert "No internal test runner" in main
    assert "No test/verification executor, queue, capacity" in main
    assert "The External Control Plane / Development System owns orchestration and policy" in adr57
    assert "test runner" in adr57.lower()
    assert "Development-validation demand and exact evidence" in control
    assert "ADR-057 moves the exact composition/execution of the ship-ready gate outside `ruu`" in control

    # Deep boundary regression checks: current normative text must not reconstruct a CI/test executor in the core.
    assert "## Amendment by ADR-057" in adr7
    assert "Git/convergence progression" in adr7
    assert "does not run or iterate tests, formatters, generators" in adr7
    assert "Verification that intentionally transforms repository candidate state participates in its own local fixed-point loop" not in adr7
    assert "Historical ADR-031 wording referred to a `ruu` verification queue" in adr5
    stale_core_phrases = [
        "verification jobs",
        "queued/running/stale required verification obligation",
        "queued/running/stale required verification workflow",
        "verification-capacity admission preserve",
        "verification-policy identity/version/fingerprint",
        "required verification category applicability/commands",
        "combined-candidate verification",
        "exact verification evidence",
        "resource/verification constraints",
    ]
    for marker in stale_core_phrases:
        assert marker not in main, marker
    assert "development-validation demand/evidence references" in main
    assert "Git/convergence validation, exact external development-validation demand/evidence binding" in main
    assert "missing/stale/nonterminal external development-validation prerequisite" in main
    assert "development-gate profile identity/version/fingerprint" in main
    assert "required development-validation applicability/evidence class" in main
...
def main():
    families = {
        ...
        "development_validation_ownership_boundary": audit_development_validation_ownership_boundary(),
        "checkpoint_external_validation_guard": audit_checkpoint_external_validation_guard(),
        "synthesized_candidate_validation_handoff": audit_synthesized_candidate_validation_handoff(),
        "external_validation_evidence_binding": audit_external_validation_evidence_binding(),
        "validation_demand_localization": audit_validation_demand_localization(),
    ...
    print("Ruu state-space audit v28: PASS")
